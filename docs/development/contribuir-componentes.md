...
- **Foco visible:** **todo elemento interactivo nuevo necesita un estado de foco visible** (ring o cambio de color con suficiente contraste), no solo quitar el outline por defecto del navegador. El patrón del proyecto es la clase `.foco-visible` (definida en `frontend/src/index.css`), ya aplicada en los primitivos de `components/ui/` (`Dialogo.tsx`, `tabs.tsx`), en las pantallas de asesorías (`MisMaterias`, `SesionesAsesor`, `MiHorario`) y en sus diálogos (`DialogoAgregarMateria`, `DialogoCancelar`, `DialogoNuevoBloque`, `SinRegistroAsesor`). `components/ui/Boton.tsx`, `screens/Login.tsx` y `screens/Landing.tsx` expresan el mismo outline con utilidades de Tailwind (`focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary`) —herencia de la Decisión 8 del plan de login frontend, anterior a la clase— y deberían converger sobre `.foco-visible`. El gap original que motivó esta regla —`Login.tsx` usaba `outline-none focus:border-primary`, sin anillo perceptible— quedó corregido el 2026-08-04.
