...
> ⚠️ **Discrepancia verificada en el código actual (no solo en la ADR):** en `config/settings/prod.py` se fija `JWT_AUTH_HTTPONLY = True` pero **nunca se configuran `JWT_AUTH_COOKIE`/`JWT_AUTH_REFRESH_COOKIE`** (los nombres de cookie, que dj-rest-auth requiere para efectivamente setear la cookie — sin nombre, `set_cookie()` no se ejecuta). Además, `DEFAULT_AUTHENTICATION_CLASSES` usa `JWTAuthentication` de simplejwt, no `JWTCookieAuthentication` de dj-rest-auth, en **todos** los entornos — aunque hubiera cookie, el backend no la leería para autenticar. Con la configuración actual, en prod `/api/auth/login/` devolvería `access` en el body, `refresh` vacío, y ninguna cookie: no habría forma de refrescar la sesión. **El flujo de dev (body + `localStorage` + header) es seguro para construir hoy; el flujo de cookies de prod descrito en la ADR no es funcional tal como está configurado el repo — vale la pena confirmarlo con el equipo de backend antes de integrar contra prod.**
