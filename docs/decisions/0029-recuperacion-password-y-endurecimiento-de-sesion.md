...
**1. El reset se acota a cuentas con contraseña usable.** No se agrega un campo de "tipo de cuenta" a `User`: la distinción es de comportamiento (los alumnos se dan de alta sin contraseña y entran por Google; las cuentas con contraseña se crean desde el admin) y `has_usable_password()` es el criterio verificable. `accounts.serializers.PasswordResetSerializer.validate_email` vacía `reset_form.users` en vez de levantar un error, para caer en el mismo camino que un correo desconocido: la respuesta queda indistinguible y no filtra ni qué cuentas existen ni de qué tipo son. Como medida de defensa en profundidad también se apaga `ACCOUNT_EMAIL_UNKNOWN_ACCOUNTS`: hoy es inerte en este camino —dj-rest-auth resuelve el reset con `AllAuthPasswordResetForm` (`dj_rest_auth/forms.py`), que sobreescribe `save()` y nunca pasa por el flujo de allauth que consulta este flag— pero si `allauth.urls` llegara a incluirse o dj-rest-auth cambiara de form, ese correo de auto-registro que Atenea no tiene (`AccountAdapter.is_open_for_signup` -> False) dejaría de aplicar, y hoy además reventaría con `NoReverseMatch` porque `allauth.urls` no está incluido en `config/urls.py`.
